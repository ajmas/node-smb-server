...
import IPCTree from './IpcTree.js'
...
class IPCShare extends Share {
  constructor(name: string, config: SmbShareConfig) {
    config = config || {}
   ...
   */
  connect(session: Session, shareLevelPassword: string, cb: (smbError?: SMBError | null, tree?: Tree) => void) {
    const self = this
