...
class FSTree extends Tree {
   ...
   */
  createFileInstance(name: string, callback: (error: SMBError | null, fsFile?: FSFile) => void) {
    FSFile.createInstance(name, this, callback)
  }
   ...
   */
  exists(name: string, cb: (smbError: SMBError | undefined | null, exists?: boolean) => void) {
   ...
   */
  open(name: string, cb: (smbError: SMBError | undefined | null) => void) {
   ...
   */
  list(pattern: string, cb: (smbError: SMBError | undefined | null) => void) {
    ...
    const self = this
    function evaluateFilter(next: (err?: SMBError | null, filter?: string[]) => void) {
      if (filter === '*') {
        // wildcard specified
    ...
    function createFileInstances(names: string[], next: (err?: SMBError | null, filter?: IFile) => void) {
      async.reduce<string, IFile[], SMBError>(
        names,
        [],
        (memo: IFile[] | undefined, name: string, callback: (err?: SMBError | null, files?: IFile[]) => void) => {
          self.createFileInstance(Path.join(parentPath, name), (err, file) => {
            if (err) {
              logger.error(
              ...
              // TODO see how to handle file instance that weren't created
              memo.push(file)
            }
   ...
   */
  createFile(name: string, callback: (error: SMBError | null, fsFile?: IFile) => void) {
    ...
    if (parentPath) {
      mkdirp(parentPath).then(() => {
        perflog.debug('%s Tree.createFile.fs.open', name)
        fs.open(filePath, 'wx', function (err, fd) {
          if (err) {
            callback(SMBError.fromSystemError(err, 'cannot create file due to unexpected error ' + name))
          } else {
            ...
            fs.close(fd, function (err) {
              if (err) {
                ...
                self.createFileInstance(name, callback)
              }
            })
          }
        });
   ...
   */
  createDirectory(name: string, cb: (smbError: SMBError | undefined | null) => void) {
    logger.debug('[%s] tree.createDirectory %s', this.share.config.backend, name)
    const self = this
    mkdirp(Path.join(this.share.path, name)).then(() => {
   ...
   * @param {SMBError} cb.error error (non-null if an error occurred)
   */
  delete(name: string, cb: (error?: SMBError | null) => void) {
  ...
  deleteDirectory(name: string, cb: (smbError?: SMBError | null, file?: IFile) => void): void {
    ...
    if (toDelete && toDelete != '/') {
      fs.rmdir(toDelete, (err: NodeJS.ErrnoException | null) => cb(SMBError.fromSystemError(err)))
    } else {
      cb(SMBError.fromSystemError(new Error('cannot delete root directory')))
    }
  }
   ...
   * @param {SMBError} cb.error error (non-null if an error occurred)
   */
  rename(oldName: string, newName: string, cb: (smbError: SMBError | undefined | null) => void) {
