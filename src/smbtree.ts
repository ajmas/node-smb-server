...
import IShare from './interfaces/IShare.js'
import SMBError from './smberror.js'
import SMBServer from './smbserver.js'
import { Listener } from './types/common.js'
 ...
 * @constructor
 */
class SMBTree implements ITree {
  static fidCounter = 0;
  static tidCounter = 0;
  ...
  tid: number;
  files: Record<string, SMBFile>;
  listeners: Record<string, Listener>;

  constructor(smbServer: SMBServer, smbShare: IShare, spiTree: ITree) {
  ...
  closeFile(fid: number, cb: (error?: Error | null) => void) {
   ...
   */
  openOrCreate(name: string, createDisposition: number, openTargetDirectory: boolean, cb: (smbError?: SMBError | null, file?: IFile) => void) {
    const self = this

    name = utils.normalizeSMBFileName(name)

    function create(callback: (smbError?: SMBError | null, smbFile?: SMBFile) => void) {
      const createFn = openTargetDirectory ? self.createDirectory : self.createFile
      createFn.call(self, name, callback)
    }

    function open(callback: (error?: SMBError | null, file?: IFile) => void) {
      self.spiTree.open(name, (err?: SMBError | null, file?: IFile) => {
        if (err) {
          callback(err)
          return
        }
        if (!file) {
          callback(new SMBError(0, 'No file provided'));
          return
        }
    ...
    if (createDisposition === common.FILE_OPEN || createDisposition === common.FILE_OVERWRITE) {
      ...
      self.exists(name, (err?: SMBError | null, exists?: boolean) => {
        if (err) {
          cb(err)
          return
        }
   ...
   * @param {SMBFile} cb.file opened file
   */
  open(name: string, cb: (smbError?: SMBError | null, file?: IFile) => void) {
    const self = this
   ...
   */
  reopen(name: string, fid: number, cb: (smbError?: SMBError | null, file?: IFile) => void) {
    const self = this
    this.spiTree.open(utils.normalizeSMBFileName(name), (err, file) => {
      if (err) {
        cb(err)
      } else {
        const result = new SMBFile(file, self, common.FILE_OPENED, fid)
   ...
   */
  list(pattern: string, cb: (smbError?: SMBError | null, smbFile?: IFile) => void) {
    const npattern = utils.normalizeSMBFileName(pattern)
    const self = this
    this.spiTree.list(npattern, function (err?: SMBError | null, files?: IFile[]) {
      if (err) {
        cb(err)
      } else if (!files) {
        cb(null, []);
   ...
   */
  createDirectory(name: string, cb: (smbError?: SMBError | null, smbFile?: SMBFile) => void) {
    const self = this
    const nname = utils.normalizeSMBFileName(name)

    this.spiTree.createDirectory(nname, (err?: SMBError | null, file?: SMBFile) => {
      if (err) {
        cb(err)
        return
      }
   ...
   */
  deleteDirectory(name: string, cb) {
    const self = this
    const nname = utils.normalizeSMBFileName(name)
  ...
  disconnect() {
    ...
    Object.keys(this.listeners).forEach(listenerId => {
      this.cancelChangeListener(listenerId);
    });

    // delegate to spi
    this.spiTree.disconnect((err) => {
      if (err) {
   ...
   */
  refresh(folderPath: string, deep: boolean, cb: (error?: SMBError | null) => void) {
    const self = this
   ...
   */
  registerChangeListener(mid: number, file: SMBFile, deep: boolean, completionFilter: number, cb: (action: number, name: string, newName?: string) => void) {
    const self = this
    ...
    listener.autoRefreshTimer = setTimeout(function () {
      // dummy change notification to force client to refresh
      const adjustedPath = path.join(listener.path, '/') // append trailing / to folder path (to make sure the proper listener is selected)
      self.notifyChangeListeners(common.FILE_ACTION_MODIFIED as number, adjustedPath)
   ...
   */
  notifyChangeListeners(action: number, name: string, newName?: string) {
    function trimListenerPath(name: string, listener: Listener) {
      return name.substr(listener.path.length + (listener.path === '/' ? 0 : 1))
    }

    function getSearchPredicate(path: string) {
      return function (listener: Listener, mid?: string): boolean {
    ...
    if (action === common.FILE_ACTION_RENAMED && newName) {
      ...
      if (listener || listenerNew) {
        if (listener && listener === listenerNew) {
          ...
          listener.cb(common.FILE_ACTION_RENAMED_OLD_NAME as number, trimListenerPath(name, listener))
        } else {
          // there's only a listener for new name
          listenerNew.cb(
            common.FILE_ACTION_RENAMED_NEW_NAME as number,
            trimListenerPath(newName, listenerNew)
          )
        }
      }
   ...
   */
  cancelChangeListener(mid: string) {
   ...
   */
  clearCache(cb: (error?: string | SMBError | null) => void) {
    const self = this
    if (self.spiTree) {
