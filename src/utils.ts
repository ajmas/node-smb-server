 ...
 */
export function lookupMimeType(path: string, fallback: string) {
  return mime.getType(path) ?? fallback;
}
 ...
 */
export function systemToSMBTime(ms: number) {
 ...
 */
export function smbToSystemTime(l: Long) {
 ...
 */
export function systemToLegacySMBDate(ms: number) {
 ...
 */
export function systemToLegacySMBTime(ms: number) {
  const date = new Date(ms)
 ...
 * @return {Number} number of milliseconds since Jan 1, 1970, 00:00:00 UTC.
 */
export function legacySMBToSystemDateTime(smbDate: number, smbTime: number): number {
 ...
 */
export function readTimestamp(buf: Buffer, pos: number) {
 ...
 */
export function parseFEAList(buf: Buffer, pos: number) {
  if (buf.length - pos < 4) {
    return null
  }
  ...
  pos += 4
  buf = buf.subarray(pos, pos + sizeOfListInBytes - 4)
  pos = 0
  ...
  while (pos < buf.length - 5) {
    ...
    ea.name = buf.subarray(pos, pos + nameLength).toString('ascii')
    pos += nameLength + 1
    // non-zero-terminated ascii string
    ea.value = buf.subarray(pos, pos + valueLenght).toString('ascii')
    pos += valueLenght
 ...
 */
export function extractUnicodeBytes(buf: Buffer, pos: number) {
  let off = pos
 ...
 */
export function extractAsciiBytes(buf: Buffer, pos: number) {
  let off = pos
...
export function calculatePadLength(offset: number, alignment: number) {
 ...
 */
export function normalizeSMBFileName(name: string) {
  name = name.replace(/\\/g, '/')
...
export function bufferEquals(a: Buffer, b: Buffer) {
 ...
 */
export function stripParentPath(path: string, parentPath: string) {
  if (path === parentPath) {
    return ''
  }
...
export function rawUUIDToString(buf: Buffer) {
...
export function parseRawUUID(buf: Buffer) {
  ...
  off += 1
  uuid.node = buf.subarray(off, off + 6)
  return uuid as UUID;
}

export function rawUUIDFromString(str: string) {
  ...
  const b4 = Buffer.from(parts[3].substring(0, 2), 'hex')
  const b5 = Buffer.from(parts[3].substring(2), 'hex')
