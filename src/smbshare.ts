 ...
 * @constructor
 */
class SMBShare implements IShare {
  name = '';
  description = '';
  path = '';
  ...
  constructor(smbServer: SMBServer, spiShare: IShare) {
    this.smbServer = smbServer
    this.spiShare = spiShare
  }

  // TODO maybe a mistake
   ...
   */
  connect(session: ISession, shareLevelPassword: string, cb: (smbError?: SMBError | null, smbTree?: ITree) => void) {
    const self = this
    this.spiShare.connect(session, shareLevelPassword, (err?: SMBError | null, tree?: ITree) => {
      if (err) {
        cb(err)
      } else {
