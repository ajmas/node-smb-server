class Asn1Obj {
  ...
  /* jslint bitwise: true */
  isPrimitive() {
    return this.tag_ !== null && (this.tag_ & 0x20) === 0 // If 6th bit === 1, it's constructed.
  }
  ...
  #traverseAndCreate(node) {
    if (node.primitive) {
      ...
      for (i = 0; i < node.children.length; i++) {
        const buffer: Buffer = this.#traverseAndCreate(node.children[i])
        buffers.push(buffer)
