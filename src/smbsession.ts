...
import SMBServer from "./smbserver";

/**
 * Represents an SMB session established by <code>SESSION_SETUP_ANDX</code>
 */
class SMBSession implements ISession {
  static uidCounter: number = 0;
  static sidCounter: number = 0;
  smbServer: SMBServer;
  spiSession: ISession;
  accountName: string;
  primaryDomain: string;
  uid: number;
  searches: Record<number, { sid: number}> = {};

  constructor(smbServer: SMBServer, accountName: string, primaryDomain: string, spiSession: ISession) {
