...
function decode (flags: number, flags2: number) {
  return {
...
function encode (flgs) {
  ...
  ret.flags =
    (flgs.reply ? consts.FLAGS_REPLY : 0) |
  ...
  ret.flags2 =
    (flgs.pathnames && flgs.pathnames.long && flgs.pathnames.long.enabled
      ? consts.FLAGS2_IS_LONG_NAME
      : 0) |
    (flgs.pathnames && flgs.pathnames.long && flgs.pathnames.long.supported
      ? consts.FLAGS2_KNOWS_LONG_NAMES
      : 0) |
    ...
    (flgs.security && flgs.security.signature && flgs.security.signature.enabled
      ? consts.FLAGS2_SECURITY_SIGNATURE
      : 0) |
    (flgs.security && flgs.security.signature && flgs.security.signature.required
      ? consts.FLAGS2_SECURITY_SIGNATURE_REQUIRED
      : 0) |
