...
import { NTLMv2Object } from './types/common.js'
 ...
 * @return {Buffer}
 */
function createLMHash (pwd: string) {
  ...
  const parts: Buffer[] = []
  let des = crypto.createCipheriv('des-ecb', expandKey(keyBuf.subarray(0, 7)), '')
  parts.push(des.update(LM_MAGIC))
  des = crypto.createCipheriv('des-ecb', expandKey(keyBuf.subarray(7)), '')
  parts.push(des.update(LM_MAGIC))
...
function calculateLMResponse (lmHash, challenge) {
  ...
  let des = crypto.createCipheriv('des-ecb', expandKey(hash.subarray(0, 7)), '')
  chunks.push(des.update(challenge))
  des = crypto.createCipheriv('des-ecb', expandKey(hash.subarray(7, 14)), '')
  chunks.push(des.update(challenge))
  des = crypto.createCipheriv('des-ecb', expandKey(hash.subarray(14)), '')
  chunks.push(des.update(challenge))
 ...
 * @return {Buffer}
 */
function calculateNTLMResponse (ntlmHash, challenge): Buffer {
  // grow 16-byte hash to 21-bytes and zero-pad
  const hash = Buffer.alloc(21)
  ntlmHash.copy(hash)
  hash.fill(0x0, 16)
...
function validateLMv2Response (lm2Response, ntlmHash, userName, domainName, challenge) {
  ...
  const nonce = lm2Response.subarray(16)
  const resp = calculateLM2Response(hash, nonce, challenge)
  return utils.bufferEquals(resp, lm2Response)
}

function validateNTLMResponse (ntlmResponse, ntlmHash, challenge): boolean {
...
function parseNTLMv2Blob (blob: Buffer) {
  ...
  const obj: Partial<NTLMv2Object> = {}
  let off = 0
  obj.signature = blob.subarray(off, 4)
  off += 4
  obj.reserved = blob.subarray(off, off + 4)
  off += 4
  obj.timeStampRaw = blob.subarray(off, off + 8)
  ...
  obj.nonce = blob.subarray(off, off + 8)
  off += 8
  obj.unknown = blob.subarray(off, off + 4)
  off += 4
  ...
  while (type !== EOL) {
    ...
    off += 2
    obj.info[type] = blob.subarray(off, off + len)
    off += len
  ...
  obj.unknown2 = blob.subarray(off, off + 4)
  off += 4
  if (off < blob.length) {
    obj.unknownTrailer = blob.subarray(off)
  }
  return obj
}

function validateNTLMv2Response (ntlm2Response: Buffer, ntlmHash: Buffer, userName: string, domainName: string, challenge: Buffer) {
  ...
  const hash = createNTLM2Hash(ntlmHash, userName, domainName)
  const blob = ntlm2Response.subarray(16)
  const blobObj = parseNTLMv2Blob(blob)
  if (!utils.bufferEquals(BLOB_SIGNATURE, (blobObj as NTLMv2Object).signature)) {
    logger.warn(
      'invalid NTLMv2 blob signature: expected: %s, actual: %s',
      BLOB_SIGNATURE.toString('hex'),
      blob.subarray(0, 4).toString('hex')
    )
    return false
  }
  ...
  return { blob, blobObj } as { blob: Buffer, blobObj: NTLMv2Object };
}
