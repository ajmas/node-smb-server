...
import IPCShare from './backends/ipc/IpcShare.js'
import FSShare from './backends/fs/FsShare.js'
import Authenticator from './spi/authenticator.js';
import IServer from './interfaces/IServer.js';
import { SmbServerConfig } from './types/common.js';
import IAuthenticator from './interfaces/IAuthenticator.js';
 ...
 * @constructor
 */
class SMBServer extends EventEmitter implements IServer {
  ...
  constructor(config: SmbServerConfig, authenticator?: Authenticator) {
    ...
    this.domainName = ((config && config.domainName) || '') as string;
    this.hostName = os.hostname()
    this.nativeOS = os.type() + ' ' + os.release()
    this.nativeLanMan = common.NATIVE_LANMAN as string
    this.config = (config && _.cloneDeep(config)) || {}
    this.authenticator = authenticator || new DefaultAuthenticator(config);

    // init shares
    for (let [name, shareCfg] of Object.entries(config.shares)) {
      ...
      const share = new SMBShare(this, new Share(name, shareCfg))
      this.shares[name] = share
      const shareEvents = share.getEvents()

      const forwardEvent = (eventName: string) => {
        share.on(eventName, (arg: unknown) => {
          logger.debug(
    ...
    // add IPC$ share
    this.shares.IPC$ = new SMBShare(this, new IPCShare('IPC$', undefined))
    ...
    this.tcpServer.on('connection', (socket: Socket) => {
      ...
      this.connections[socket.id] = new SMBConnection(socket, this)
    })
