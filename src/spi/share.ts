 ...
 */
class Share extends EventEmitter implements IShare {
  config: SmbShareConfig ;
  name: string = '';
  description: string = '';
  path: string = '';

  constructor(name: string, config: SmbShareConfig = ({} as SmbShareConfig)) {
    // call the super constructor to initialize `this`
    super()

    this.config = config;
    this.name = name;
    this.description = (this.config.description || '') as string;
  }
   ...
   */
  connect(session: Session, shareLevelPassword: string, callback: (smbError?: SMBError | null) => void) {
    process.nextTick(() => {
      callback(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED));
    })
  }
}
