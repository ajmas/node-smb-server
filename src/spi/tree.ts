...
import { unicodeNormalize, unicodeEquals } from '../utils.js'
...
class Tree implements ITree {
   ...
   */
  list(pattern: string, cb: (error: SMBError | null | undefined, files?: IFile[]) => void) {
    process.nextTick(function () {
      cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED))
    })
  }
   ...
   */
  createFile(name: string, cb: (error: SMBError | undefined |null, destFile?: IFile) => void) {
    process.nextTick(function () {
      cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED))
    })
  }
   ...
   */
  createDirectory(name: string, cb: (error: SMBError | undefined | null, destFile?: IFile) => void) {
    process.nextTick(function () {
      cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED))
    })
  }
   ...
   * @param {SMBError} cb.error error (non-null if an error occurred)
   */
  delete(name: string, cb: (error?: SMBError | null, file?: IFile) => void) {
    process.nextTick(function () {
      cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED))
    })
  }
   ...
   * @param {SMBError} cb.error error (non-null if an error occurred)
   */
  deleteDirectory(name: string, cb: (error?: SMBError | null, file?: IFile) => void) {
    process.nextTick(function () {
      cb(new SMBError(ntstatus.STATUS_NOT_IMPLEMENTED))
    })
  }
   ...
   */
  unicodeNormalize(str: string) {
    if (!this.config.noUnicodeNormalize) {
      return unicodeNormalize(str)
    } else {
      return str
    }
  }
   ...
   */
  unicodeEquals(str1: string, str2: string) {
    if (!this.config.noUnicodeNormalize) {
      return unicodeEquals(str1, str2)
    } else {
      return str1 === str2
    }
  }
   ...
   */
  clearCache(cb: (error?: string | SMBError) => void) {
    cb()
  }
}
